...
func (hc *HealthChecker) checkAll(ctx context.Context) {
	...
	if err != nil {
		log.Printf("[health-checker] failed to fetch backends from redis: %v", err)
		return
	}
...
func (hc *HealthChecker) recordResult(ctx context.Context, backend string, success bool) {
	...
	if success {
		...
		if !wasHealthy && state.consecutiveSuccesses >= hc.healthyThreshold {
			state.healthy = true
			hc.mu.Unlock()
			log.Printf("[health-checker] backend %s became healthy, adding to active set", backend)
			if err := hc.redis.AddBackend(ctx, backend); err != nil {
				log.Printf("[health-checker] failed to SADD %s: %v", backend, err)
			}
			return
		}
	} else {
		...
		if wasHealthy && state.consecutiveFails >= hc.unhealthyThreshold {
			state.healthy = false
			hc.mu.Unlock()
			log.Printf("[health-checker] backend %s became unhealthy after %d consecutive failures, removing from active set",
				backend, hc.unhealthyThreshold)
			if err := hc.redis.RemoveBackend(ctx, backend); err != nil {
				log.Printf("[health-checker] failed to SREM %s: %v", backend, err)
			}
			return
		}
	}
